...
```mermaid
graph TD
    A[Gebruiker] -->|Onderzoeksvraag| B[Main Agent]
    B -->|Internet Search| C[HybridSearchTool]
    C -->|Keuze| D[Tavily API]
    C -->|Keuze| E[Multi-Search API]
    B -->|Delegeert| F[Research Sub-Agent]
    B -->|Delegeert| G[Critique Sub-Agent]
    F -->|Gebruikt| C
    G -->|Valideert| H[Bevindingen]
    B -->|Genereert| I[final_report.md]
```
...
sequenceDiagram
    ...
    U->>M: Onderzoeksvraag
    M->>M: Plan maken (TODO's)
    M->>R: Delegeer onderzoek
    R->>S: Zoek informatie
    S-->>R: Resultaten
    R-->>M: Bevindingen
    M->>C: Valideer bevindingen
    C->>S: Extra verificatie
    S-->>C: Resultaten
    C-->>M: Feedback
    M->>M: Genereer rapport
    M-->>U: final_report.md
```
...
```mermaid
flowchart LR
    A[Vraag] --> B{Main Agent}
    B --> C[Plan TODO's]
    C --> D[Research Sub-Agent]
    D --> E[Internet Search]
    E --> F[Resultaten normaliseren]
    F --> D
    D --> G[Bevindingen]
    G --> H[Critique Sub-Agent]
    H --> I{Valid?}
    I -->|Nee| E
    I -->|Ja| J[Rapport genereren]
    J --> K[final_report.md]
```
